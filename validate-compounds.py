...
def main():
    # read compounds
    compounds = set(read_lines("compounds.txt"))
    ...
    # count parts
    partCounts = collections.Counter(itertools.chain.from_iterable(
        compoundTuples
    ))
    print(
    ...
    parts.update(
        l.split(",")[0].strip("'- ")
        for l in read_lines("generated-lists/words.csv")
    )
    ...
    print("Possible compounds (parts that consist of 2-3 other parts):")
    results = set()
    for comp in parts:
        for pos1 in range(2, len(comp) - 2 + 1):
            ...
            if part1 in parts and part1 not in EXCLUDE_PARTS:
                ...
                for pos2 in range(pos1 + 2, len(comp) - 2 + 1):
                    ...
                    if (
                        part2 in parts and part3 in parts
                        and part2 not in EXCLUDE_PARTS
                        and part3 not in EXCLUDE_PARTS
                        and not (
                            part1[-1] == part2[0] and part1[-1] in "aeiouyäö"
                        ) and not (
                            part2[-1] == part3[0] and part2[-1] in "aeiouyäö"
                        )
                    ):
    ...
    print(
        "Possible non-compounds (compounds with a fancy Latin/Greek part):"
    )
    print(",".join(
    ...
    # e.g. {"maaliero": {"maa_liero", "maali_ero"}, ...}
    partPairsByNoBoundary = {}
